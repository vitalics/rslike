...
- a36f2e3: iter: add `ParIter` (concurrent pipelines via `Promise.all`) and `WorkerParIter` (CPU-parallel pipelines on worker threads) with dedicated `./par-iter` and `./worker-par-iter` subpath exports.
...
- 2c89d8f: Initial implementation
